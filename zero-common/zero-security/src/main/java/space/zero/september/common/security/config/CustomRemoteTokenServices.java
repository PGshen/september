 ...
 */
@Component
@ConditionalOnProperty(prefix = "security.oauth2", name = {"resource", "client"})
public class CustomRemoteTokenServices implements ResourceServerTokenServices {
    ...
    public OAuth2Authentication loadAuthentication(String accessToken) throws AuthenticationException, InvalidTokenException {
        ...
        if (map.containsKey("code") && ((String)map.get("code")).endsWith(ErrorCode.P512.code() + "")) {
            if (logger.isDebugEnabled()) {
                logger.debug("check_token returned error: " + map.get("data"));
            }
            throw new InvalidTokenException(accessToken);
        }
